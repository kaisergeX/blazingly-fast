...
fn main() {
    ...
    println!("\n\n========= Challenge =========\n\n");
    let handle_challenge = thread::spawn(move || {
        for msg in rx_challenge {
            println!("Challenge channel: Received {}", msg);
        }
    });

    for value in 0..10 {
        println!("Main thread: sending {} to challenge channel", value);
        pause_ms(500);
        tx_challenge.send(value).unwrap();
    }

    drop(tx_challenge);
    handle_challenge.join().unwrap();
